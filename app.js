(() => {
  ...
  function assignedTripMembers() {
    if (!state.data) return [];
    const tripId = String(state.data.trip.tripId || "").trim().toUpperCase();
    const allowedIds = new Set((state.data.assignments || []).filter((assignment) => !assignment.tripId || String(assignment.tripId).trim().toUpperCase() === tripId).map((assignment) => String(assignment.travellerId || "").trim().toUpperCase()).filter(Boolean));
    if (state.travellerId) allowedIds.add(String(state.travellerId).trim().toUpperCase());
    return state.data.members.filter((member) => member.travellerId && allowedIds.has(String(member.travellerId).trim().toUpperCase()));
  }
  function visibleTripMembers() { return isAdmin() ? state.data.members : assignedTripMembers(); }
  ...
  function renderPeople() {
    ...
    const cards = members.map((member) => `<article class="person ${member.travellerId ? "personal-access" : "shared-only"}"><i>${esc(initials(member.name))}</i><div><h3>${esc(member.name)}</h3><p>${isAdmin() ? (member.travellerId ? `Traveller ID · ${esc(member.travellerId)}` : (member.role === "Organiser" ? "Trip organiser · Administrator access" : "No personal PIN yet")) : "Approved by Administrator for this trip"}</p></div><span>${esc(member.role)}</span><div class="person-access-badge ${member.travellerId ? "enabled" : "pending"}">${isAdmin() ? (member.travellerId ? "PERSONAL PIN ENABLED" : (member.role === "Organiser" ? "ADMIN" : "PIN REQUIRED")) : "APPROVED TRAVELLER"}</div><footer><span><small>PAID FOR TRIP</small><b>${money.format(totals[member.name] || 0)}</b></span><span class="person-footer-actions">${isAdmin() && member.travellerId ? `<button class="pin-reset-control" data-reset-member-pin="${esc(member.id)}" aria-label="Edit PIN for ${esc(member.name)}">✎ Edit PIN</button>` : ""}${isAdmin() && !member.travellerId && member.role !== "Organiser" ? `<button class="pin-reset-control" data-give-pin="${esc(member.id)}" aria-label="Create PIN for ${esc(member.name)}">＋ Create PIN</button>` : ""}${isAdmin() && member.role !== "Organiser" ? `<button class="delete-control" data-delete data-sheet="Members" data-id="${esc(member.id)}">Remove</button>` : ""}</span></footer></article>`).join("");
    return `${heading("YOUR TRAVEL GROUP", isAdmin() ? "Travellers and personal PINs" : "Approved travellers", isAdmin() ? "Assign traveller profiles and edit their personal PINs." : "Only travellers approved by the Administrator for this trip are shown here.", "travellers")}<div class="share-banner"><div><h3>${isAdmin() ? "Separate secure access for every traveller" : `${members.length} approved ${members.length === 1 ? "traveller" : "travellers"}`}</h3><p>Trip ID <b>${esc(state.data.trip.tripId)}</b> · ${isAdmin() ? "Choose exactly who is allowed to join this trip" : "This list follows the Administrator’s trip assignments"}</p></div>${isAdmin() ? `<span class="banner-actions"><button data-add="travellers">＋ Add travellers with PINs</button><button data-all-trips>All trips</button><button data-security>⚿ Security</button></span>` : `<span class="readonly-label">APPROVED GROUP</span>`}</div><div class="people-grid">${cards || `<div class="empty-approved-travellers"><b>No travellers approved yet</b><p>The Administrator has not assigned any traveller profile to this trip.</p></div>`}</div>`;
  }
